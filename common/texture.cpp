...
GLuint loadBMP(const char* imagePath) {
    ...
    if (fread(header, 1, 54, file) != 54) {
        fclose(file);
        throw runtime_error("Not a correct BMP file1");
    }
    // A BMP files always begins with "BM"
    if (header[0] != 'B' || header[1] != 'M'){
        fclose(file);
        throw runtime_error("Not a correct BMP file2");
    }
    // Make sure this is a 24bpp file
    if (*(int*)&(header[0x1E]) != 0) {
        fclose(file);
        throw runtime_error("Not a correct BMP file3");
    }
    if (*(int*)&(header[0x1C]) != 24) {
        fclose(file);
        throw runtime_error("Not a correct BMP file4");
    }
